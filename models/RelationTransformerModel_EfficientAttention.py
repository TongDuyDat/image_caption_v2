...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, boxes, tgt, src_mask, tgt_mask):
        ...
        return self.decode(self.encode(src, boxes, src_mask), src_mask, tgt, tgt_mask)
...
class EfficientAttention(nn.Module):
    def __init__(self, num_heads, dim, sr_ratio=1):
        super().__init__()
...
def box_attention(
    query, key, value, box_relation_embds_matrix, mask=None, dropout=None
):
    """
...
class BoxMultiHeadedAttentionEff(nn.Module):
    ...
    def forward(self, input_query, input_key, input_value, input_box, H, W, mask=None):
        ...
        relative_geometry_weights_per_head = [
            l(flatten_relative_geometry_embeddings).view(box_size_per_head)
            for l in self.WGs
        ]
...
class RelationTransformerModelEfficient(CaptionModel):
    ...
    def make_model(
        ...
        model = EncoderDecoder(
            Encoder(EncoderLayer(d_model, c(eff_attn), c(ff), dropout), N),
    ...
    def __init__(self, opt):
        super(RelationTransformerModelEfficient, self).__init__()
        self.opt = opt
        ...
        self.box_trignometric_embedding = getattr(
            opt, "box_trignometric_embedding", True
        )
    ...
    def _forward(self, fc_feats, att_feats, boxes, seq, att_masks=None):
        ...
        att_feats, boxes, seq, att_masks, seq_mask = self._prepare_feature(
            att_feats, att_masks, boxes, seq
        )
    ...
    def _sample_beam(self, fc_feats, att_feats, boxes, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            tmp_memory = (
                memory[k : k + 1]
                .expand(*((beam_size,) + memory.size()[1:]))
                .contiguous()
            )
            tmp_att_masks = (
                att_masks[k : k + 1]
                .expand(*((beam_size,) + att_masks.size()[1:]))
                .contiguous()
                if att_masks is not None
                else None
            )
            ...
            for t in range(1):
                ...
                logprobs, state = self.get_logprobs_state(
                    it, tmp_memory, tmp_att_masks, state
                )

            self.done_beams[k] = self.beam_search(
                state, logprobs, tmp_memory, tmp_att_masks, opt=opt
            )
    ...
    def _sample_(self, fc_feats, att_feats, boxes, att_masks=None, opt={}):
        ...
        if sample_max:
            with torch.no_grad():
                seq_, seqLogprobs_ = self._sample_(
                    fc_feats, att_feats, boxes, att_masks, opt
                )
        ...
        for i in range(self.seq_length):
            out = self.model.decode(
                memory, att_masks, ys, subsequent_mask(ys.size(1)).to(att_feats.device)
            )
        ...
        assert (
            seqLogprobs * ((seq_ > 0).float()) - seqLogprobs_ * ((seq_ > 0).float())
        ).abs().max() < 1e-5, "logprobs doens't match"
